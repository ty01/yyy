...
//            useDefaultStyle: true,                  //是否使用默认highchart主题样式
        ...
        var arrMaxYAxis = [];                       //标识多个Y坐标轴的最大值，适用于多Y轴情况
        var yAxisCount = 1;
...
//        $(this).css('width', options.width);
		...
		if ($.isArray(cOptions.yAxis)){
            yAxisCount = cOptions.yAxis.length; 
            for(var i = 0; i < yAxisCount; i++){
                arrMaxYAxis[i] = 0;             //初始化各Y轴最大值
            }
        }
		...
		if ($.isArray(cOptions.yAxis)){
			//当有多个Y轴时，取第一个为主Y轴
			mainYAxis = cOptions.yAxis[0];
			for(var i in cOptions.yAxis){
				formatYAxis(cOptions.yAxis[i], i);
			}
		}else{
			mainYAxis = cOptions.yAxis;
			formatYAxis(cOptions.yAxis, 0);
		}		
		...
		switch(options.labelFormat){
			...
			case 1:
				...
				cOptions.plotOptions.series.dataLabels.formatter = cOptions.plotOptions.series.dataLabels.formatter || function(){
					var thisSeries = this.point.series;
					var dataFormat = thisSeries.dataFormat || thisSeries.yAxis.userOptions.dataFormat || mainYAxis.dataFormat;
					return formatValue(dataFormat, this.y);
				}
				break;
			case 2:
				cOptions.plotOptions.series.dataLabels.enabled = true;
		...
		if (cOptions.chart.type != 'pie'){
            cOptions.tooltip.formatter = cOptions.tooltip.formatter || function(){
				...
				$.each(this.points, function(i, point) {
                    var dataFormat = point.series.dataFormat || point.series.yAxis.userOptions.dataFormat || mainYAxis.dataFormat;
...
//                    symbolWidth: 12 
    //                align: 'left'
            ...
            if (theme == 'qyh'){
                ...
                defOptions = $.extend(true, defOptions, qyhOptions);
            }
		...
		//格式化指定Y轴
		function formatYAxis(yAxis, index){

			yAxis.dataFormat = yAxis.dataFormat || options.dataFormat;
			//Y轴最大、最小值
			if (options.yMin != null && options.yMax != null){
				yAxis.min = options.yMin;
				yAxis.max = options.yMax;
			}

			//如果是百分比格式，则限制Y轴最大值为100%（即可所有数据的最大值大于95%时才有效）
			if (yAxis.dataFormat == 3){
                if (arrMaxYAxis[index] && arrMaxYAxis[index] > 95){
                    yAxis.max = 100;
                }
//                maxY > 95){
//				yAxis.max = 100;
			}
			if (yAxis.dataFormat == 5){
                if (arrMaxYAxis[index] && arrMaxYAxis[index] > 0.95){
                    yAxis.max = 1;
                }
...
//				yAxis.max = 1;
			...
			if (cOptions.tooltip.valueSuffix){
				yAxis.labels.formatter = function(){
					var value = (yAxis.dataFormat == 5)?  Highcharts.numberFormat(this.value * 100, 0) : this.value;
					return value + cOptions.tooltip.valueSuffix;
				};
			}
			if (yAxis.dataFormat == 4){
				//转成时分秒格式H:mm:ss或百分比
				yAxis.labels.formatter = function(){
					return formatValue(yAxis.dataFormat, this.value);
				};
			}
			//Y轴数据只加百分比符号，而不必强制保留两位小数
			if (yAxis.dataFormat == 5 || yAxis.dataFormat == 3){
				yAxis.labels.formatter = function(){
					var value = (yAxis.dataFormat == 5)?  Highcharts.numberFormat(this.value * 100, 0) : this.value;
					return value + '%';
				};
...
*/
            //设死marker导致不能配置，去掉它
            options.theme != 'wechat' && isNotPieChart() && (hcPoint.marker = hcPoint.marker || {}, typeof(hcPoint.marker.enabled) == 'undefined' && (hcPoint.marker.enabled = false));

            //判断是否有Y值，并设置Y轴最大值，总数等
            var y = hcPoint.y;
            if (y != null){
                hasData = true;
                counter++;
                maxY = maxY > y? maxY : y;
                sumY += hcPoint.y;
                arrMaxYAxis[yAxis] = arrMaxYAxis[yAxis] > y? arrMaxYAxis[yAxis] : y;
            } 
        ...
        for(var i in hcData){
            ...
            if (point.y != null){
                //设死marker导致不能配置，去掉它
                options.theme != 'wechat' && isNotPieChart() && (point.marker.enabled = false);
