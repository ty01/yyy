	...
	var Util = {
		...
		validate: function(validate, value){
			...
			if(!value || !$.trim(value).length){
				return validate.require ? false : true;
			}
...
(function(){
	...
	function Participates(bind){
		...
		this.setConfig = function(val){
			...
			if(config.properties.attach){//嵌套风格
				...
				// select2 美化下拉框
				sendTo.select2($.extend(true, {minimumResultsForSearch:-1}, config.select2||{}));
				//只有一个选项，隐藏下拉框
		...
		function render(div, orgId, node, processInstId, nullable){
			...
			if(node.exts.attrs.customParticipant=="true"){
				return;
			}
		...
		function initbindFormValue(node){
			var participateFrom = node.exts.attrs.participateFrom;
			...
			if(participateFrom){
				if(participateFrom.indexOf("formValue")>-1){
					...
					var widgetdom = $("#"+widgetid) || $("div[name=" + widgetid +"]");
					var orgid = widgetdom.sui().getValue();
					if(orgid){
		...
		function bindFormValue(node){
			...
			if(participateFrom){
				if(participateFrom.indexOf("formValue")>-1){
					...
					var widgetid = participateFrom.substring( participateFrom.indexOf("[")+1 , participateFrom.indexOf("]") );
					var $dom = $("#"+widgetid) || $("div[name=" + widgetid +"]");
					if($dom.length){
	...
	function FileUpload(bind){
		...
		var dom = bind || $('<div></div>').get(0);
		dom.zoo = this;
		var items, controlId, tempFileId, photos, btnId, uploadDo;
		this.setConfig = function(val){
			config = val;
		...
		this.setValue = function(val){
			...
			if(val){
				...
				$.each(fileKeys, function(i, key){
					/*lazyLoading.run({
						id: key,
						exe: function(dataAll){
							var data = dataAll[key];
							var item = createFileItem(key, data.value.fileName, data.value.fileLength);
							items.append(item);
							onchange();
						}
					});*/
		...
		function render(){
			...
			var resetControl = function(formData){
				if(formData){
					...
					control = control.clone(true, true);
				}
			}
			if(uploadDo.canUpload){
				...
				if(window.FormData){
					...
					control.on("change", function(){
						...
						$.ajax({
							...
							success : function(data) {  
								...
								if(resultData.error){
									layer.alert("上传失败，" + resultData.error,{icon: 2, title:'提示',closeBtn: 0});
								}else{
					...
					var loadingView = $('<div class="upload-loading" /><div style="display:none;"><iframe name="'+winName+'" src="#"></iframe></div>');
					div.append(loadingView);
					loadingView = loadingView.first();
					var url = settings.uploader + "?formScript=parent.document.getElementById('"+formId+"').uploadResult.call";
					var form = $('<form id="'+formId+'" action="'+url+'" method="post" enctype ="multipart/form-data" target="'+winName+'"></form>');
		...
		function createFileItem(fileKey, fileName, fileSize){
			...
			view.click(function(){
				view.attr({
					"href": SUI.settings().uploadify().downloadPath + input.val(),
					"target":"_blank"
				});
			});
	...
	function OpinionOne(bind){
		...
		this.setValue = function(val){
			...
			if(value.activityId  && config.mode == "editable" && $(".sui-participates").length > 0){
				var partiConfig = $(".sui-participates").sui().getConfig();
				if(partiConfig.properties.attach){//嵌套风格
					$(".sui-participates").find("input[type=radio][value="+value.activityId+"]").iCheck("check");
				}else{
					$(".sui-participates").find("select").val(value.activityId);//.trigger("change");
				}
			}
		}
		function getMyKey(){
        ...
        this.setConfig = function(val){
				   ...
				   if(list.length>0){
					   ...
					   $.each(list, function(i, item){
						   if(item.WORKITEMTYPE =="M" && item.ASSISTANTNAME){
							   ...
							   if(item.END_TIME.length>3){
								   item.END_TIME = item.END_TIME.substring(0,item.END_TIME.length-3);
							   }
					   ...
					   if(!hasHis){
						   ...
						   $(dom).html('<div style="text-align:center;" class="lookOpinionHistory"><a  class="btn btn-success lookOpinionHistoryBtn" href="javascript:void(0);">点击查看</a></div>'+
								   '<div class="form-table">' + tr + "</div>"+
								   '<div style="text-align:center;" class="hideOpinionHistory"><a  class="btn btn-success hideOpinionHistoryBtn" href="javascript:void(0);">点击隐藏</a></div>');
			...
			if(config.properties.expand == "true"){
				load();
					...
					$(dom).html('<div style="text-align:center;"><a class="btn btn-success" href="javascript:void(0);">点击查看</a></div>');
				...
				$.ajax({
					...
					success: function(data) {
					   ...
					   $(dom).html('<div style="text-align:center;" class="lookOpinionHistory"><a  class="btn btn-success lookOpinionHistoryBtn" href="javascript:void(0);">点击查看</a></div>'+
							   '<div class="form-table">' + tr + "</div>"+
							   '<div style="text-align:center;" class="hideOpinionHistory"><a  class="btn btn-success hideOpinionHistoryBtn" href="javascript:void(0);">点击隐藏</a></div>');
					   $('.lookOpinionHistory').hide();
					   
					   $('.lookOpinionHistoryBtn').click(function(){
						   $('.lookOpinionHistory').hide();
						   $('.form-table', dom).show();
						   $('.hideOpinionHistory').show();
					   });
					   $('.hideOpinionHistoryBtn').click(function(){
						   $('.lookOpinionHistory').show();
						   $('.form-table', dom).hide();
						   $('.hideOpinionHistory').hide();
					   });
	...
	function Input(bind){
        ...
        this.setConfig = function(val){
			...
			if(mode == "readOnly" || mode == "view"){
				...
				$tpl.html(("undefined"==typeof parseData.prefix?"":(parseData.prefix+" ")) + '<span class="value">' +_value+ '</span>' + ("undefined"==typeof parseData.suffix?"":(" "+parseData.suffix)));
	...
	**/
	...
	function Datatable(bind){
		var dom = bind || $("<div />")[0], config, value, columns;
		dom.zoo = this;
		var $table, $tableHead, $tableBody, $addBtn;
		this.init = function (val){
			this.setConfig(val);
		}
        ...
        this.setConfig = function(val){
        	var that = this;
			config = val;
        	if(!$.isPlainObject(config.dataTable) || !config.dataTable.columns){
        		return;
        	}
			this.name = config.properties.name;
			this.title = config.properties.title;
			...
			$tableWrap = $('<div class="datatable-wrapper" />').css({
				'overflow': 'auto'
			});
        ...
        function buildTr(_data){
        	...
        	var $tr = $('<tr class="table-row" id="'+id+'" />');
        	var mode = config.mode || "view";
        	$.each(columns, function(index, column) {
        		var $td = $('<td />', column.attrs);
        		var component = $.extend(true, {}, column.component, {
        			isui: true,
				...
				success: function(data){
					if(data.allow=="true"){
						...
						$(dom).click(function(){
							     ...
							     entity[config.lcztName.name]=config.lcztName.value;
							     entity = wf2.merge(entity);
							     	$.ajax({
							 			type: "POST",
