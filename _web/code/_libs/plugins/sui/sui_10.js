...
(function(){
	...
	(function(){
		...
		function build(element, type){
			...
			$(el).attr("sui",isui ? "isui" : "true");
			var conf = el.zoo.parse(element);
			el.zoo.init(conf);
			if(conf.mode!="editable"){
				//$('input,textarea',el).attr('unselectable', "on");
			}
...
(function(){
	...
	function Participates(bind){
		...
		function decision(node, linkExt, attach){
			...
			$(".sui-opinion[mode=editable], .sui-multiopinion[mode=editable], .sui-opinionone", dom).each(function(){
				...
				com.setOpinion(linkExt.autoFill || "");
			});
		...
		function initbindFormValue(node){
			...
			if(participateFrom){
				if(participateFrom.indexOf("formValue")>-1){
					...
					if(orgid){
						render(status.div ,orgid ,status.node ,status.status, status.nullable);
					}
				}
			}
		}
	...
	function FileUpload(bind){
		...
		var dom = bind || $('<div></div>').get(0);
		dom.zoo = this;
		var items, controlId, tempFileId, photos, btnId;
		this.setConfig = function(val){
			config = val;
			...
			if(params.canUpload){
				...
				if(window.FormData){
					...
					control.on("change", function(){
						...
						if(!validate){
							layer.alert("上传失败，允许上传文件类型为：" + config.properties.ext ,{icon: 2, title:'提示',closeBtn: 0});
							return;
						}
					...
					var url = settings.uploader + "?formScript=parent.document.getElementById('"+winName+"').uploadResult.call";
					var form = $('<form id="'+winName+'" action="'+url+'" method="post" enctype ="multipart/form-data" target="'+winName+'"></form>');
	...
	function Datatable(bind){
        ...
        this.getValue = function(js){
        	...
        	$(".table-row", $tableBody).each(function(index, el) {
        		var item = {};
        		$("[sui=isui]", el).each(function(index, _el) {
        			item[$(_el).attr('name').replace(/trid-\d+/,"")] = _el.zoo.getValue();
        		});
        		_value.push(item);
        	});
        ...
        function _parseData(element){
        	...
        	var _data = {
        		...
        		addButton: $(el).attr('addButton') || "添加数据",
        ...
        function _validate(){
        	...
        	$("[sui=isui]", $tableBody).each(function(index, el) {
        		if(nopass(el)){
        			result = false;
        		}
        	});
        ...
        function addBtn(data){
        	if(value.length || (data&&data.length) || config.unaddable){
