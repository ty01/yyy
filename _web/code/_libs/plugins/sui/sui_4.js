...
(function(){
	...
	function Participates(bind){
		...
		this.setConfig = function(val){
			...
			if(config.properties.attach){//嵌套风格
					...
					if(!hasHide){
						...
						if(node.options){
							$.each(node.options, function(i, opt){
								...
								id = "sui-parties-" + id;
								options += '<label for="'+id+'"><input id="'+id+'" type="radio" name="'+opName+'" value="'+node.activity.id+'" displayName="'+opt.name+'" m-property="'+opt.property+'" m-value="'+opt.value+'"/>'+opt.name+'</label>';
							});
						}else{
							var id = SUI.Util.getSequence("parties");
							id = "sui-parties-" + id;
							options += '<label for="'+id+'"><input id="'+id+'" type="radio" name="'+opName+'" value="'+node.activity.id+'" displayName="'+node.displayName+'"/>'+node.displayName+'</label>';
						}
					}
				});
				var html = '<div class="m_title"><div class="radio-group">'+options+'</div></div><div class="m_body"></div><div style="margin-top:10px;overflow:hidden;"><div class="col-xs-2" style="padding:0; width:100px;">下一步处理者：</div><div class="wf-parties col-xs-6"></div></div>';
				...
				radios.iCheck(SUI.settings().iCheck({
				...
				radios.on('ifChecked',function(){
					...
					var activityId = selectRadio.val();
					displayName = selectRadio.attr("displayName");
					if(selectRadio.attr("m-property")){
						optionExt = optionExt||{};
						optionExt.property = selectRadio.attr("m-property");
						optionExt.value = selectRadio.attr("m-value");
					}
					var node;
					$.each(tree.children,function(i, curNode){
						if(curNode.activity.id == activityId){
							node = curNode;
						}
					});
					parties.parent().show();
				...
				$.each(tree.children, function(i, node){
					...
					if(!hasHide){
						if(node.options){
							$.each(node.options,function(i, opt){
								var option = $('<option value="'+node.activity.id+'">'+opt.name+'</option>');
								option.prop("node", node);
								option.prop("opt", opt);
								sendTo.append(option);
							});
						}else{
							var option = $('<option value="'+node.activity.id+'">'+node.displayName+'</option>');
							option.prop("node", node);
							sendTo.append(option);
						}
					}
				...
				sendTo.change(function(){
					...
					if(opt){
						optionExt = optionExt||{};
						optionExt.property = opt.property;
						optionExt.value=opt.value;
					}
	...
	function MultiOpinion(bind){
		...
		this.setValue = function(val){
			...
			for(var key in data){
				...
				html += '<div style="line-height: 1.2;">'+item.userName+'的意见：'+item.opinion+'<br/><span style="display:inline-block; width:40px;"></span>——部门:'+item.userOrgName+' 日期：'+ dt +'</div>';
			}
			div.append($(html));
			$(dom).trigger('change');
		}
	...
	function Opinion(bind){
		...
		this.setValue = function(val){
			...
			value = SUI.Util.parseJSON(value, true);
			extAttrs = value.extAttrs;
			var item = value;
			var html = "";
			var dt = new Date();
			dt.setTime(item.date -0);
			dt = SUI.Util.dateFormat(dt, "yyyy-MM-dd hh:mm");
			html += item.opinion+'<br/><span style="display:inline-block; width:40px;"></span>——'+item.userName+'：'+ dt;
			label.html(html);
			opinion.val(value.opinion).trigger('change');
		}
		...
		this.setConfig = function(val){
			...
			var html = '';
			var maxlength = config.maxlength ? ' maxlength="'+config.maxlength+'"':"";
			html += '<div style="line-height: 1.2;"></div><textarea class="form-control" rows="3"'+maxlength+'></textarea>';
			$(dom).html(html);
			var template = $(dom);
			opinion = template.find("textarea");
			label = template.find("div");
			...
			$.ajax({
				...
				success: function(data) {
				   ...
				   $.each(list, function(i, item){
					   if(item.DEALRESULT || item.DEALOPINION){
						   tr += '<div class="form-tr clearfix"><div class="form-td col-xs-12"><div class="form-item"><div class="item-name">'+
						   item.WORKITEMNAME+'：</div><div class="item-value">'+item.DEALRESULT+'<br/>'+item.DEALOPINION+'</div></div></div></div>'
					   }
				   });
