	...
	function Participates(bind){
		...
		this.setConfig = function(val){
			...
			if(tree.children.length == 1){
				//sendTo.close();
				sendTo.next().hide();
				sendTo.after('<span>'+tree.children[0].activity.name+'</span>');
			}
	...
	***/
	...
	function DateRange(bind){
        ...
        this.setConfig = function(val){
			...
			var title = parseData.properties.title || "";
			var name = parseData.properties.name || "";
			var startDate = moment(config.value[0]),
	        	endDate = moment(config.value[1]);
	        var _startVal = config.value[0] || "",
				_endVal =  config.value[1] || "";
