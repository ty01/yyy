...
(function(){
	...
	function Participates(bind){
			...
			if(config.properties.attach){//嵌套风格
				...
				var html = '<div class="m_title"></div><div class="m_body"></div>'+
					'<div style="margin-top:10px;overflow:hidden;">'+
						'<div style="padding:0;float:left;">下一步：<span class="m_step"></span> &nbsp;&nbsp;&nbsp;&nbsp; <span class="m_parti_title">经办人：</span></div>'+
						'<div class="wf-parties col-xs-6" style="padding-left:0;"></div>'+
					'</div>';
		...
		//决断
		function decision(node, linkExt){
			linkExt = linkExt || {};
			if(linkExt.decision){
				setExt(linkExt.name);
			}else{
		...
		function createFileItem(fileKey, fileName){
			...
			delBtn.click(function(){
				layer.confirm("是否确认删除？",{icon: 3, title:'提示'},function(){
					var pid = $(delBtn).parent().data("pid");
					if(pid){
						var pidx = APP.inArrayObj('pid', pid, photos.data);
						if(pidx!==-1){
							photos.data.splice(pidx, 1);
						}
					}
